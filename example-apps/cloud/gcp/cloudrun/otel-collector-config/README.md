...
*IMPORTANT*: The collector and its resource detector `gcp` does not set a core resource attribute `service.instance.id` but `faas.instance` as an instanceid. for normalised standard portable telemetry the resource/instance id processor has been added to these files.
