...
These configs only support gRPC OTLP receivers as to work with code-based OpenTelemetry app instrumentation and not AWS SDK telemetry like the xray library. They are designed for PRODUCTION use with trace filters, memory limit, collector telemetry (core signals for alerting) and tail sampling to reduce load/cost of telemetry.
