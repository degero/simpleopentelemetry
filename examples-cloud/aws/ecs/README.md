# AWS SimpleOpenTelemetry ASPNetCore example
...
The sample adotcollector-local-X.yml configs are designed for PRODUCTION with health check, trace filters, memory limit and tail sampling to reduce load/cost of telemetry. If using xray sampling (appsetting UseXraySampler - default on) remove the tail sampling from the config.
...
1. In /: docker build --no-cache -t <yourtagname> . 
...
1. Deploy the terraform with terraform init/plan/apply
...
1. Use terraform destroy in the /infra folder to destroy the ECS+EC2 instance (you may need to terminate/delete the ec2 instance to unblock)
